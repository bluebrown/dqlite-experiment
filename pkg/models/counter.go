...
var CounterWhere = struct {
	...
	ID:    whereHelperint64{field: "\"counter\".\"id\""},
	Count: whereHelperint64{field: "\"counter\".\"count\""},
}
...
var (
	counterAllColumns            = []string{"id", "count"}
	counterColumnsWithoutDefault = []string{}
	counterColumnsWithDefault    = []string{"id", "count"}
	counterPrimaryKeyColumns     = []string{"id"}
	counterGeneratedColumns      = []string{"id", "count"}
)
...
func (q counterQuery) One(ctx context.Context, exec boil.ContextExecutor) (*Counter, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: failed to execute a one query for counter")
	}
...
func (q counterQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to count counter rows")
	}
...
func (q counterQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "models: failed to check if counter exists")
	}
...
func Counters(mods ...qm.QueryMod) counterQuery {
	mods = append(mods, qm.From("\"counter\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"counter\".*"})
	}
...
func FindCounter(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*Counter, error) {
	counterObj := &Counter{}
	...
	query := fmt.Sprintf(
		"select %s from \"counter\" where \"id\"=?", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: unable to select from counter")
	}
...
func (o *Counter) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("models: no counter provided for insertion")
	}
	...
	if !cached {
		...
		wl = strmangle.SetComplement(wl, counterGeneratedColumns)
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"counter\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"counter\" %sDEFAULT VALUES%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to insert into counter")
	}
...
func (o *Counter) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("models: unable to update counter, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE \"counter\" SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update counter row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by update for counter")
	}
...
func (q counterQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update all for counter")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to retrieve rows affected for counter")
	}
...
func (o CounterSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"counter\" SET %s WHERE %s",
...
func (o *Counter) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no counter provided for upsert")
	}
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("models: unable to upsert counter, could not build update column list")
		}
		...
		cache.query = buildUpsertQuerySQLite(dialect, "\"counter\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert counter")
	}
...
func (o *Counter) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete from counter")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by delete for counter")
	}
...
func (q counterQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete all from counter")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by deleteall for counter")
	}
...
func (o *Counter) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindCounter(ctx, exec, o.ID)
	if err != nil {
		return err
	}
...
func (o *CounterSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"counter\".* FROM \"counter\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, counterPrimaryKeyColumns, len(*o))
...
func CounterExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"counter\" where \"id\"=? limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "models: unable to check if counter exists")
	}
